    ...
    */
    ...
    private double setAllowedhDist(final Player player, final boolean sprinting, 
        ...
        // TODO: Ensure higher speed only for flight of stairs, rather
            ...
            friction = 0.0;
            if (!Double.isInfinite(mcAccess.getHandle().getFasterMovementAmplifier(player))) hAllowedDistance *= 0.90;
        }
        ...
        // Collision tolerance for entities (1.9+)
                ...
                && hAllowedDistance < 0.38 && data.liftOffEnvelope == LiftOffEnvelope.NORMAL) {
            tags.add("hcollision");
            ...
            if (thisMove.from.inWater || !thisMove.from.inLava) { 
                ...
                if (data.liqtick < 5 && lastMove.toIsValid) {
                    if (!lastMove.from.inLiquid) {
                        if (lastMove.hDistance * 0.98 > thisMove.hDistance) {
                            hAllowedDistance = lastMove.hDistance * 0.98;
                        }
                    } 
                    else if (lastMove.hAllowedDistance * 0.98 > thisMove.hDistance) {
                        hAllowedDistance = lastMove.hAllowedDistance * 0.98;
                    }
                }
                ...
                if (from.isInWaterLogged() && data.insideMediumCount <= 1 // (Actually observed was 0)
                    && !from.isSubmerged(0.78) && (!lastMove.from.inLiquid || !pastMove2.from.inLiquid)
        ...
        // TODO: !sfDirty is very coarse, should use friction instead.
            ...
            if (thisMove.from.onGround) {
                ...
                if (!thisMove.to.onGround) {
                    ...
                    hAllowedDistance = (lastMove.hDistance > 0.29 ? 0.8 : 0.29 + (ServerIsAtLeast1_13 ? 0.155 : 0.0)) +
                ...
                // OnGround
                    ...
                    if (lastMove.toIsValid && lastMove.hDistance > 0.0) {
                       hAllowedDistance = data.noSlowHop < 7 ?
                                        ...
                                        (lastMove.hAllowedDistance * (0.8 + 0.095 * ++data.noSlowHop)) : lastMove.hAllowedDistance;
                    }
                ...
                if (data.noSlowHop == 1 && lastMove.toIsValid) {
                    ...
                    hAllowedDistance = lastMove.hAllowedDistance * 0.8 * cc.survivalFlyBlockingSpeed / 100D;
        ...
        if (useBaseModifiers) {
            ...
            if (attrMod == Double.MAX_VALUE) {
                ...
                if (!Double.isInfinite(speedAmplifier) && useBaseModifiersSprint) {
                    hAllowedDistance *= 1.4D + 0.2D * speedAmplifier;
                }
            }
            else {
                ...
                if (sprinting && hAllowedDistance < 0.34 && cc.sfSlownessSprintHack
                    && (
                ...
                if (!useBaseModifiersSprint) {
                    ...
                    if (!Double.isInfinite(speedAmplifier)) {
                        hAllowedDistance /= attrMod;
                        hAllowedDistance *= attrMod - 0.24D * speedAmplifier;
                    }
                }
            }
        }
        ...
        if (data.keepfrictiontick > 0) {
            if (!BridgeEnchant.hasSoulSpeed(player)) {
                ...
                hAllowedDistance = Math.max(hAllowedDistance, lastMove.hAllowedDistance * 0.99);
            }
        }
    ...
    private double[] vDistAir(final long now, final Player player, final PlayerLocation from, 
        ...
        final double jumpGainMargin      = 0.008; // TODO: Model differently, workarounds where needed. 0.05 interferes with max height vs. velocity (<= 0.47 gain).
        ...
        if (lastMove.toIsValid && Magic.fallingEnvelope(yDistance, lastMove.yDistance, data.lastFrictionVertical, 0.0)) {
            ...
            if (toOnGround) {
                ...
                if (yDistance > cc.sfStepHeight && yDistance - cc.sfStepHeight < 0.00000009
            ...
            if (lastMove.yDistance >= -Math.max(Magic.GRAVITY_MAX / 2.6, 1.8 * Math.abs(yDistance))
        ...
        // Teleport/join/respawn.
            ...
            // Allow jumping.
                ...
                vAllowedDistance = maxJumpGain + jumpGainMargin;
                if (lastMove.to.onGround && vAllowedDistance < 0.7) vAllowedDistance = maxJumpGain + jumpGainMargin;
                // Allow stepping
        ...
        if (!pData.hasPermission(Permissions.MOVING_SURVIVALFLY_STEP, player) && yDistance > 0.0 
            ...
            if (totalVDistViolation > 0.0) {
                ...
                // Attempt to use velocity.
                    ...
                    vDistanceAboveLimit = Math.max(vDistanceAboveLimit, Math.max(totalVDistViolation, 0.7));
                ...
                if (!data.sfLowJump && !data.sfNoLowJump && lastMove.toIsValid && lastMove.yDistance > 0.0 
                    ...
                    if (setBackYDistance > 0.0 && setBackYDistance < minJumpHeight) {
                        ...
                        if (
                            ...
                            && lastMove.yDistance <= Magic.GRAVITY_MAX * 2.39 && lastMove.yDistance > 0.0
        ...
        if (InAirPhase && cc.survivalFlyAccountingV) {
            ...
            if (MovingUtil.isCollideWithHB(from, to, data) && thisMove.yDistance < 0.0 && thisMove.yDistance > -0.28) {
                data.vDistAcc.clear();
                data.vDistAcc.add((float) -0.208);
            }
                ...
                data.vDistAcc.add((float) yDistance);
                    ...
                    && !(lastMove.from.inLiquid && Math.abs(yDistance) < 0.35
        ...
        if (count0 > 0) {
            ...
            if (count1 > 0) {
                ...
                if (sc0 > sc1 - 3.0 * Magic.GRAVITY_VACC) {
                    ...
                    if (yDistance <= -1.09 && sc1 < -8.0 && sc0 < -8.0) {
    ...
    private double[] hDistAfterFailure(final Player player, 
        ...
        if (cc.survivalFlyResetItem && hDistanceAboveLimit > 0.0 && data.sfHorizontalBuffer <= 0.8 && tags.contains("usingitem")) {
            tags.add("itemreset");
            // Handle through nms
        ...
        if (cc.trackBlockMove && hDistanceAboveLimit > 0.0 && hDistanceAboveLimit < 1.028) {
            ...
            if (Math.abs(xDistance) > 0.45 && Math.abs(xDistance) < 1.03
                && from.matchBlockChange(blockChangeTracker, data.blockChangeRef, xDistance < 0 ? Direction.X_NEG : Direction.X_POS, 0.08)) {
                hAllowedDistance = thisMove.hDistance; // MAGIC
                hDistanceAboveLimit = 0.0;
            }
            else if (Math.abs(zDistance) > 0.488 && Math.abs(zDistance) < 1.028
                    && from.matchBlockChange(blockChangeTracker, data.blockChangeRef, zDistance < 0 ? Direction.Z_NEG : Direction.Z_POS, 0.08)) {
                hAllowedDistance = thisMove.hDistance; // MAGIC
                hDistanceAboveLimit = 0.0;
            }
        }
    ...
    private double[] vDistClimbable(final Player player, final PlayerLocation from, final PlayerLocation to,
        ...
        final double jumpHeight = LiftOffEnvelope.NORMAL.getMaxJumpHeight(0.0) + (data.jumpAmplifier > 0 ? (0.9 + data.jumpAmplifier - 1.0) : 0.0);
        final double maxJumpGain = data.liftOffEnvelope.getMaxJumpGain(data.jumpAmplifier) + 0.008;
    ...
    private double[] vDistBush(final Player player, final PlayerMoveData thisMove, 
        ...
        final double yDistance = thisMove.yDistance;
        final PlayerMoveData lastMove = data.playerMoves.getFirstPastMove();
        final double jumpGainMargin = 0.008;
        double vAllowedDistance, vDistanceAboveLimit;
        ...
        // (Falling speed is static, however if falling from high enough places, it can depend on how fast one "dives" in.)
            ...
            else vAllowedDistance = -Magic.GRAVITY_MIN * data.lastFrictionVertical - 0.0008;
            vDistanceAboveLimit = yDistance < vAllowedDistance ? Math.abs(yDistance - vAllowedDistance) : 0.0;
        }
    ...
    */
    private double[] vDistPowderSnow(final double yDistance, final PlayerLocation from, final PlayerLocation to, 
        ...
        if (yToBlock <= cc.yOnGround) {
            vAllowedDistance = data.liftOffEnvelope.getMinJumpGain(data.jumpAmplifier, 1.8);
        }
        else {
        ...
        final double yDistDiffEx = yDistance - vAllowedDistance;
        final boolean violation = fall ? Math.abs(yDistDiffEx) > 0.08 : Math.abs(yDistance) > Math.abs(vAllowedDistance);
